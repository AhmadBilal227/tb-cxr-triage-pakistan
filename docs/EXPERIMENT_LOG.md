...
| 05-24 | Re-extraction (letterbox geom + box/zone tensors) @95% sens, deterministic code | AUROC ≈0.92 (op-point change only, not capability); spec ~0.55–0.70; worst-fold lead | _PENDING (run bfm0gn9h5, ~20%)_ | — | watch: AUROC should NOT jump (only threshold moved); if it does → drift |
