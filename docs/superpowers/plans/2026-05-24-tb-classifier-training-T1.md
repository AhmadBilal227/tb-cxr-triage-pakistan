...
### Accuracy-improvement roadmap — commercial + SOTA-informed (2026-05-24)
A study of the leading products (Google TB DLS, CAD4TB, qXR, Lunit, JF CXR-1) + frozen-foundation SOTA. Their accuracy comes from three things we can't copy now (data scale — qXR ~4.4M / Lunit ~3M / Google 550k; NLP-mined or microbiology-confirmed labels; semi-supervised noisy-student pretraining — Rad-DINO/ELIXR is our substitute) and three architecture/calibration choices we CAN. **Binding constraint = cross-site generalization, not capacity** → prioritize complementary-signal + shortcut-reduction; gate every capacity-add on frozen-threshold cross-source sensitivity (drop anything that only helps in-distribution). Note: `extract_features.py` currently caches only the CLS vector — Tier-1 #1/#2 are "build the fusion + patch head the plan already promised."

**Tier 1 — do first (cheap, high ROI, anti-shortcut):**
1. **Build the TXRV fusion** (not yet in code): concat Rad-DINO CLS (768) ⊕ TorchXRayVision DenseNet (1024 + 18 pathology logits). Run TXRV on ITS OWN preprocessing ([-1024,1024], 224, no CLAHE) — two preprocessing paths. ~+0.02–0.05 LODO AUC, highest confidence; the 18 supervised logits are the pathology-grounded anti-shortcut signal.
2. **Attention-pooled head over Rad-DINO PATCH tokens** (GLoRI/gated-attention-MIL over the 37×37 grid), not CLS-only. Documented frozen-Rad-DINO lift 0.723→0.783 AUROC (+6). Concentrates gain on focal/miliary/apical lesions + yields free localization. Cache a compact patch summary (CLS + mean + max + top-k attention pool), not the raw grid, to keep the npz small. Keep the head tiny + heavily regularized (overfit risk) and gate on LODO.
3. **Lung-crop soft front-end** (Google gains big from a 138-mask lung crop): dilated soft-mask + bbox crop with margin (NOT hard-zero), removes scanner borders/markers that drive LODO shift.
4. **Loss/head:** class-balanced (effective-number) + label smoothing (ε≤0.1); log-odds late-fusion ensemble of a Rad-DINO head ⊕ a TXRV head (more shortcut-robust than pure concat — blend by LODO).

**Tier 2 — next:**
5. **Per-site / score-normalized threshold calibration** — every product confirms fixed thresholds don't transfer (CAD4TB cutoff 63 Nepal vs 48 Cameroon). Report AUC (threshold-free) as headline; pick the operating point on the held-out distribution at 90% sens; Mondrian conformal re-fit per site.
6. **Flip-free TTA** (rotation/scale/brightness multi-crop, log-odds averaged) for reported numbers — but ship 1–2× in-browser (TTA multiplies deployment cost).
7. **Optional 3rd frozen encoder: Google CXR-Foundation / ELIXR** — strongest TB-specific external evidence (frozen embeddings non-inferior to radiologists for TB from 45 images, and beat fine-tuning). Moderate integration (TF, gated). Diminishing returns after Rad-DINO⊕TXRV; if adding one more, make it this.

**Tier 3 / deprioritize:** higher-res/tiling (miliary-only, breaks bit-exact preprocessing + heavier ONNX), gradient-boosted-tree head (ensemble member only), **LoRA/last-block fine-tune (LAST RESORT — ELIXR shows frozen beats fine-tune for TB; high overfit/shortcut risk on 3–4 sites; defer to Phase B with in-domain data)**, frozen-probe augmentation (marginal), self-distillation/pseudo-labeling (Phase B, in-domain unlabeled only).
