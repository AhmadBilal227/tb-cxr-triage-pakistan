...
- Asymmetric-evidence escalation (M26): `src/lib/pipeline/asymmetricEvidence.ts` — THIRD NO_TB→ABSTAIN escalator alongside sequelaeEscalation (M19) and vlmEscalation (M21). When the local pipeline returns very-low `tb_prob` but `box_evidence_max` AND `top_pathology_score` (max of 5 TB-relevant TXRV labels) both fire high, escalate to ABSTAIN. Thresholds derived on the M24 blind-set diagnostic — see the rule's header for the paper trail. Composition order in `combineLocalIntoAdjudication`: base verdict → verifier disagree → sequelae → asymmetric-evidence → final (each a one-way escalator; they compose cleanly).
- M24 evidence panels (local-mode only — VLM-primary path emits no enrichment): `src/components/details/BoxEvidenceHeatmap.tsx` (8x8 viridis heatmap above Details), `ZonalBars.tsx` (7 ZonalSoftOR per-zone bars inside Details), `PathologyList.tsx` (18 TXRV severity-colored chips), `ClinicianReport.tsx` (on-demand gpt-as-interpreter narrative; calls `src/lib/providers/gptInterpreter.ts` once on click; verdict-bound prompt; load-bearing disclosure *"Narrative interpretation only — does not change the verdict."*). Shared `colorScale.ts` (viridis ramp + severity buckets).
